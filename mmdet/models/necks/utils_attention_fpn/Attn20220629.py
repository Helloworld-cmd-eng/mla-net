...
class NeckViTModule(nn.Module):
    def __init__(self, num_patches,dim, depth, heads, mlp_dim, pool='cls',dim_head=64, dropout=0.5, emb_dropout=0.5):
        ...
        self.sp_transformer = Transformer(dim, depth, heads, dim_head, mlp_dim, dropout)

        self.fc1=nn.Linear(dim*num_patches,dim)
        self.fc2=nn.Linear(dim*num_patches,dim)
        self.fc3=nn.Linear(dim*num_patches,dim)
...
if __name__=='__main__':
    v = NeckViTModule(num_patches=9, dim=64, heads=4,depth=2,mlp_dim=64, pool='cls', dim_head=32,dropout=0.5)
