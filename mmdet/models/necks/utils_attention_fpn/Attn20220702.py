...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        return [m(e) for e,m in zip(out.chunk(self.n_out,-1),self.to_out)]
...
class NeckViTModule(nn.Module):
    def __init__(self, num_patches,n_out,dim, heads,dim_head=64, dropout=0.5, emb_dropout=0.5):
        ...
        self.fc=nn.ModuleList(nn.Sequential(nn.Linear(dim*num_patches,dim)) for _ in range(n_out))
    ...
    def forward(self, feat_x):
        ...
        out=[]
        for e,m in zip(x_rich_list,self.fc):
            out.append(m(e.flatten(1,2)))
